...
tshark -2 -nr "$FILE_NAME" -R "$PORTS_TO_SERVER || $PORTS_TO_AUTH1" -T fields -e frame.len | wc -l
tshark -2 -nr "$FILE_NAME" -R "$PORTS_TO_SERVER || $PORTS_TO_AUTH1" -T fields -e frame.len | awk '{sum+=$1}END{print sum}'
tshark -2 -nr "$FILE_NAME" -R "$PORTS_FROM_SERVER || $PORTS_FROM_AUTH1" -T fields -e frame.len | wc -l
tshark -2 -nr "$FILE_NAME" -R "$PORTS_FROM_SERVER || $PORTS_FROM_AUTH1" -T fields -e frame.len | awk '{sum+=$1}END{print sum}'
